module Kr = struct
  ...
  let schedule_to_string = function 
    | Quarterly (q, i) -> "(" ^ quarter_to_string q ^ " " ^ string_of_int i ^ ")"
    | Rolling -> "(rolling)"
...
module Item = struct 
  ...
  let of_project : Project.t -> t list = fun { project; objectives } ->
    let of_objective : Objective.t -> t list = fun { title = objective; krs } -> 
